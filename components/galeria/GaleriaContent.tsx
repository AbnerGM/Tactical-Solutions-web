    ...
    return (
        <div className="bg-[#F9F8F6] min-h-screen text-tactical-dark font-sans pb-16">
                ...
                <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-4">
                        ...
                        {CATEGORY_TABS.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`px-3.5 py-2 rounded-lg text-xs font-bold transition-all duration-200 whitespace-nowrap flex items-center gap-2 cursor-pointer ${isActive
                                ...
                                >
                                    <span>{tab.label}</span>
                                    <span
                                        className={`px-1.5 py-0.2 rounded-full text-[10px] font-mono ${isActive ? "bg-white/20 text-white" : "bg-zinc-200 text-zinc-800"
                                            }`}
                                    >
                                        {count}
                                    </span>
                    ...
                    <div className="relative w-full lg:w-72">
                        <input
                            ...
                            className="w-full pl-9 pr-8 py-2 bg-zinc-50 border border-zinc-300 rounded-lg text-xs text-tactical-dark placeholder-zinc-400 focus:outline-none focus:border-tactical-green focus:ring-1 focus:ring-tactical-green transition-all"
                        />
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                ...
                <div className="flex items-center justify-between mb-5 pb-2 border-b border-zinc-200 text-xs">
                ...
                {/* Grilla visual limpia de fotos */}
                {filteredPhotos.length === 0 ? (
                    <div className="bg-white rounded-xl p-12 text-center border border-zinc-200 max-w-md mx-auto my-12 shadow-xs">
                        ...
                        <button
                            ...
                            className="px-4 py-2 bg-tactical-green hover:bg-[#072c1d] text-white text-xs font-bold uppercase rounded-lg transition-colors cursor-pointer"
                        >
                            Ver todas las fotos
                        </button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {filteredPhotos.map((photo, index) => (
                            <div
                                ...
                                className="group relative aspect-4/3 rounded-xl overflow-hidden bg-tactical-dark border border-zinc-200 hover:border-tactical-green cursor-pointer transition-all duration-300 shadow-xs hover:shadow-xl"
                            >
                                {/* Foto de la galería */}
                                <Image
                                    src={photo.image}
                                ...
                                <div className="absolute top-2.5 left-2.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <span className="bg-tactical-dark/90 text-white text-[10px] font-mono font-bold uppercase tracking-wider px-2.5 py-1 rounded border border-tactical-green/60">
                                        {photo.categoryLabel}
                                    </span>
                                </div>

                                {/* Botón de expandir / pantalla completa en esquina superior derecha */}
                                <div className="absolute top-2.5 right-2.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-tactical-green hover:bg-[#072c1d] text-white w-8 h-8 rounded-lg flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                ...
                <div className="mt-12 text-center">
                    ...
                    <Link
                        href="/contacto"
                        className="inline-block px-5 py-2.5 bg-tactical-green hover:bg-[#072c1d] text-white font-bold text-xs uppercase tracking-wider rounded-lg transition-colors shadow-xs"
                    >
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                            <span className="bg-tactical-green text-white font-mono text-xs font-bold px-2.5 py-1 rounded">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                onClick={handleClose}
                                className="w-9 h-9 rounded-lg bg-zinc-800 hover:bg-tactical-red text-white flex items-center justify-center transition-colors cursor-pointer text-base font-bold"
                    ...
                    >
                        ...
                        <button
                            onClick={handlePrev}
                            className="absolute left-2 sm:left-4 z-20 w-11 h-11 sm:w-13 sm:h-13 rounded-lg bg-zinc-900/90 hover:bg-tactical-green text-white flex items-center justify-center transition-all duration-200 cursor-pointer border border-zinc-700 shadow-xl group"
                        ...
                        <button
                            onClick={handleNext}
                            className="absolute right-2 sm:right-4 z-20 w-11 h-11 sm:w-13 sm:h-13 rounded-lg bg-zinc-900/90 hover:bg-tactical-green text-white flex items-center justify-center transition-all duration-200 cursor-pointer border border-zinc-700 shadow-xl group"
                        ...
                        <div className="flex items-center justify-center gap-1.5 overflow-x-auto max-w-3xl mx-auto py-1 scrollbar-none">
                            {filteredPhotos.map((thumb, idx) => (
                                <button
                                    ...
                                    className={`relative w-12 h-10 sm:w-14 sm:h-11 rounded overflow-hidden border-2 transition-all shrink-0 cursor-pointer ${idx === selectedIndex
